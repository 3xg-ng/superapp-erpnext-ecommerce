...
def add_to_cart(user_id, item_id, item_name, quantity=1):
    
    try:
        # Check if the item already exists in the user's cart
        cart_item = frappe.db.get_value("Cart", {"user_id": user_id, "item_id": item_id}, "quantity")
        ...
        frappe.db.commit()
        return create_response(SUCCESS, f"Item {item_id} added to cart successfully!")

    except frappe.DuplicateEntryError:
        return create_response(SERVER_ERROR, "Duplicate entry found. Please try again.")
    except Exception as e:
        frappe.log_error(f"Error adding item {item_id} to cart for user {user_id}: {str(e)}", "Add to Cart Error")
...
def update_cart_quantity(user_id, item_id, quantity):
    ...
    try:
        ...
        if not cart_item:
            return create_response(NOT_FOUND, f"Item {item_id} not found in the cart.")
        ...
        if quantity == 0:
            frappe.db.sql("""
                ...
                WHERE user_id = %s AND item_id = %s
            """, (user_id, item_id))
            message = f"Item {item_id} removed from the cart."
        else:
        ...
        frappe.log_error(f"Error updating cart item {item_id} for user {user_id}: {str(e)}", "Update Cart Error")
